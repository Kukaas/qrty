...
export default function InfoModal({ isOpen, onClose }: InfoModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-md bg-white border border-zinc-200 rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90dvh]">
        {/* Header */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-3 sm:py-4 border-b border-zinc-200 bg-zinc-50/70">
          <div className="flex items-center gap-2.5">
            <LogoMark size={28} />
            <div>
              <h2 className="text-xs sm:text-sm font-bold text-zinc-900 leading-none font-mono">qrty // Architecture</h2>
              <span className="text-[10px] sm:text-xs text-zinc-500 font-mono">Tactical 3D QR System • v1.0.5</span>
            </div>
          </div>
          <button
            onClick={() => {
        ...
        {/* Content */}
        <div className="p-4 sm:p-6 flex flex-col gap-3 sm:gap-4 overflow-y-auto text-xs sm:text-sm text-zinc-700 font-mono leading-relaxed">
          ...
          <div className="grid grid-cols-1 gap-2.5 sm:gap-3">
            <div className="p-3 sm:p-3.5 rounded-2xl bg-zinc-50 border border-zinc-200 flex items-start gap-2.5 sm:gap-3">
              <Trees className="w-4 h-4 sm:w-5 sm:h-5 text-zinc-900 shrink-0 mt-0.5" />
              <div>
            ...
            <div className="p-3 sm:p-3.5 rounded-2xl bg-zinc-50 border border-zinc-200 flex items-start gap-2.5 sm:gap-3">
              <QrCode className="w-4 h-4 sm:w-5 sm:h-5 text-zinc-900 shrink-0 mt-0.5" />
              <div>
        ...
        <div className="px-4 sm:px-6 py-3 sm:py-3.5 border-t border-zinc-200 bg-zinc-50/80 flex items-center justify-between gap-3 text-xs font-mono">
          <a
            href="https://kukaass.app/#contact"
            target="_blank"
            rel="noopener noreferrer"
            className="text-zinc-600 hover:text-zinc-950 font-semibold underline flex items-center gap-1 text-[10px] sm:text-[11px] truncate"
          >
            <span className="truncate">Suggestions? Contact</span>
          ...
          <button
            ...
            className="px-3.5 sm:px-4 py-2 rounded-xl bg-zinc-900 hover:bg-black text-white text-xs font-bold font-mono cursor-pointer transition-colors shrink-0"
          >
            Acknowledge
          </button>
        </div>
